...
# -- The 'base' sub-query consolidates all relevant data from various tables, joining driver details, run statistics, course information, and car

# -- 'fast_one' is used to identify the minimum (fastest) time recorded by each driver per course, labeling any driver without a recorded time as 'dnf' .

# -- 'to_columns' pivots the course times from 'fast_one' into separate columns for each course

# -- 'to_overall' calculates the overall performance by summing the individual course times. It marks any driver with a 'dnf' status in any course as 'NotQualified' to ensure they aren't included in the ranking for prizes.

# -- 'to_rank' implements a ranking system based on the aggregated times from 'to_overall,' considering those marked as 'NotQualified' as the lowest rank, ensuring they don't mix with ranked competitors.

# -- 'to_prize' determines prize eligibility, awarding a 'cup' to the top-ranked driver and 'prize' to those ranked 2nd to 5th. Those outside the top five don't receive a prize categorization.
...
"""
# -- This query retrieves a list of drivers,
# including their ID, full name, date of birth, age, and caregiver.
# Results are sorted first by age (oldest first) and then by surname alphabetically.
    ...
    if request.method == "POST":
        ...
        # If a driver was selected from the list, retrieve and display that driver's detailed information.
            ...
            sql_query = """
                ...
                WHERE CONCAT(driver.first_name, ' ', driver.surname) = %s;
            """
    # -- This query retrieves detailed information about drivers' courses and performance, 
    # including times wd cones, for a specific driver identified by their full name.
...
def add_driver_adult():
    ...
    # For a POST request, we first handle the form submission by inserting the new driver.
        ...
        cursor.execute("""
            INSERT INTO driver ( first_name, surname, car)
            VALUES ( %s, %s, %s)
        """, (driver_firstname, driver_surname, car_num))
       # Get the id generated by the last insertion operation
...
def add_driver_junior():
    ...
    # This function fetches car information. It is use for the dropbox car info
    car_info = []   
    def fetch_car_info():
        cursor.execute("""
            SELECT car.car_num AS car_num,
    ...
    # This function fetches caregiver information. It is use for the dropbox caregiver info
    caregiver_info=[]
    ...
    # For a POST request, we first handle the form submission by inserting the new driver.
        ...
        driver_surname = request.form['driver_surname']
        car_num = request.form['car_model'].split(' ')[0]  # Getting car number
        selected_caregiver =request.form['caregiver']
       ...
       # Adjust the caregiver ID based on age and the selected caregiver.
        if age > 16 and selected_caregiver == "none":
           caregiver_id = None  # For individuals over 16, if "none" is selected, no caregiver is needed.
        elif age <= 16 and selected_caregiver:
           caregiver_id = selected_caregiver.split(' ')[0]  # For individuals 16 and under, retrieve the caregiver ID based on the selection.
        else:
            # For any cases that don't adhere to the rules, returning an error message or setting a default value.
           caregiver_id = None  
        ...
        # Insert the new driver
        cursor.execute("""
            INSERT INTO driver (first_name, surname, age, caregiver, car)
            VALUES ( %s, %s, %s, %s, %s)
        """, ( driver_firstname, driver_surname, age, caregiver_id,car_num))
